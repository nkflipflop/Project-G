        ...
        */
        ...
        private void CreateBSP(SubDungeon subDungeon)
        {
            ...
            if (subDungeon.IAmLeaf())
            {
                ...
                if (subDungeon.rect.width > Config.maxRoomSize || subDungeon.rect.height > Config.maxRoomSize ||
        ...
        private void PlaceLamps(int[,] dungeonTiles)
        {
            ...
            for (int j = Config.dungeonColumns + (2 * Config.dungeonPadding) - matrixSize;
                 j >= 0;
                 j--)
            {
                for (int i = 0;
                     i <= Config.dungeonRows + (2 * Config.dungeonPadding) - matrixSize;
                     i++)
                {
                    int mulResult = 0;
                    for (int l = 0; l < matrixSize; l++)
        ...
        public void CreateDungeon(GameManager gameManager)
        {
            ...
            rootSubDungeon = new SubDungeon(new Rect(Config.dungeonPadding, Config.dungeonPadding, Config.dungeonRows,
                Config.dungeonColumns));
            CreateBSP(rootSubDungeon);
            rootSubDungeon.CreateRoom();

            dungeonFloorPositions = new GameObject[Config.dungeonRows + (2 * Config.dungeonPadding),
                Config.dungeonColumns + (2 * Config.dungeonPadding)];
            DungeonMap = new int[Config.dungeonRows + (2 * Config.dungeonPadding),
                Config.dungeonColumns + (2 * Config.dungeonPadding)];
            objectSpawnPos = new int[Config.dungeonRows + (2 * Config.dungeonPadding),
                Config.dungeonColumns + (2 * Config.dungeonPadding)];
