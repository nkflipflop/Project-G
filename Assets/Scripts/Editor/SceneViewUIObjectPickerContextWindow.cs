...
public class SceneViewUIObjectPickerContextWindow : EditorWindow
{
    ...
    private readonly MethodInfo screenFittedRectGetter = typeof(EditorWindow).Assembly
        .GetType("UnityEditor.ContainerWindow").GetMethod("FitRectToScreen",
    ...
    private void ShowContextWindow(List<Entry> results)
    {
        ...
        position = (Rect)screenFittedRectGetter.Invoke(null,
            new object[3]
            {
                new Rect(GUIUtility.GUIToScreenPoint(Event.current.mousePosition) - new Vector2(0f, position.height),
                    position.size),
                true, true
            });
    }
        ...
        for (int i = 0; i < uiObjects.Count; i++)
        {
            ...
            if (GUI.Button(rect, uiObjectLabels[i], rowGUIStyle))
            {
                if (uiObjects[i] != null)
                {
                    ...
                    ExpandToGameObject(Selection.activeGameObject);
                }
    ...
    private static void ExpandToGameObject(GameObject obj)
    {
        EditorApplication.ExecuteMenuItem("Window/General/Hierarchy");
        var hierarchy_window = focusedWindow;
        SetExpandedRecursive(obj, true, hierarchy_window);
    }

    private static void SetExpandedRecursive(GameObject go, bool expand, EditorWindow hierarchy_window)
    {
    ...
    private static void OnSceneViewRightClicked(SceneView sceneView)
    {
        ...
        rootEntry.Children.RemoveAll((canvasEntry) =>
            canvasEntry.Children.Count == 0 && !canvasEntry.RectTransform.GetComponent<Graphic>());
    ...
    private static void CheckRectTransformRecursive(RectTransform rectTransform, Vector2 pointerPos, Camera camera,
        bool culledByCanvasGroup, List<Entry> result)
    {
    ...
    private static bool ShouldCheckRectTransform(RectTransform rectTransform, Vector2 pointerPos, Camera camera,
